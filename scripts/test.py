...
def run(measure, level):
    print(measure, level)
    try:
        fouts = [open(f"{output_dir}/{measure}_{metric}_{level}", "a", buffering = 1) for metric in ["accuracies", "auc", "pred"]]
        for i in range(6):
            df_ds = list()
            for j in range(5):
...
if __name__ == "__main__":
    ...
    measure_activity_levels = {"hour": [0.6],
                               "day": [0.7]}
    
    with Pool(processes=15) as pool:
        for measure, levels in measure_activity_levels.items():
            for level in levels:
                pool.apply_async(run, args=(measure, level))
        pool.close()
        pool.join()